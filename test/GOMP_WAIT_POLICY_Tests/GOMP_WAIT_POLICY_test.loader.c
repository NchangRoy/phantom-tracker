 ...
 *
 * Attaches three programs to the OpenMP / kernel stack:
 ...
 *      do_wait is inlined and stripped so it has no symbol. The loader
 *      uses find_do_wait.h to discover all inlined copies via disassembly
 *      heuristic and attaches a separate uprobe to each one.
 *      Each link is pinned at:
 *        /sys/fs/bpf/links/gomp_wait_do_wait_<N>
 *
 * Prerequisites:
 *   - omp_thread_reg loader must be running (omp_threads_map pinned at
 *     /sys/fs/bpf/omp_threads_map).
 *   - host_ivshmem.ko must be loaded (provides bpf_host_ivshmem_h2g_read).
 *   - Run as root.
 */
...
#define PIN_SWITCH_LINK      "/sys/fs/bpf/links/gomp_wait_sched_switch"
#define PIN_FUTEX_LINK       "/sys/fs/bpf/links/gomp_wait_futex_enter"
/* do_wait links are pinned as gomp_wait_do_wait_0, gomp_wait_do_wait_1, … */
#define PIN_DO_WAIT_FMT      "/sys/fs/bpf/links/gomp_wait_do_wait_%d"
...
int main(void)
{
    ...
    if (n_do_wait == 0) {
        fprintf(stderr,
                ...
                "         tracepoints will still be attached.\n",
                libgomp_path);
    } else {
    ...
    if (err) {
        futex_link = NULL;
        fprintf(stderr, "failed to attach tp/syscalls/sys_enter_futex: %s\n",
                strerror(-err));
        goto cleanup;
    }
    if (bpf_link__pin(futex_link, PIN_FUTEX_LINK) < 0) {
        perror("pin futex_enter link");
        goto cleanup;
    }
    printf("Attached → tp/syscalls/sys_enter_futex    (gomp_futex_enter)\n");

    /* 5c — attach one uprobe per do_wait body found */
    for (int i = 0; i < n_do_wait; i++) {
        ...
        char pin_path[128];
        snprintf(pin_path, sizeof(pin_path), PIN_DO_WAIT_FMT, i);
        if (bpf_link__pin(do_wait_links[i], pin_path) < 0) {
            fprintf(stderr, "failed to pin do_wait link [%d] at %s: %s\n",
                    i, pin_path, strerror(errno));
            goto cleanup;
        }

        printf("Attached → %s+0x%lx  [do_wait body %d] pinned at %s\n",
               libgomp_path, dw_offsets[i], i, pin_path);
    }

    if (n_do_wait == 0)
        printf("Skipped do_wait uprobe (no bodies found)\n");

    printf("\nAll programs attached. Links pinned under /sys/fs/bpf/links/.\n");
    printf("To detach, remove the pinned link files:\n");
    printf("  rm %s %s", PIN_SWITCH_LINK, PIN_FUTEX_LINK);
    for (int i = 0; i < n_do_wait; i++) {
        char pin_path[128];
        snprintf(pin_path, sizeof(pin_path), PIN_DO_WAIT_FMT, i);
        printf(" %s", pin_path);
